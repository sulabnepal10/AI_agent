...
async def search_serpapi(query: str, serpapi_key: str, start: int = 0) -> List[Dict]:
    ...
    try:
        ...
        logger.error(f"SerpAPI error for query '{query}' at start={start}: {e}")
        return []
            ...
            for list_result in list_results[:2]:  
                ...
                async with AsyncWebCrawler(config=BrowserConfig(headless=True)) as crawler:
                    ...
                    if result and result.markdown:
                        ...
                        client = AsyncOpenAI(api_key=os.environ.get("OPENAI_API_KEY", st.session_state.get("openai_key", "")))
                        response = await client.chat.completions.create(
                            model="gpt-4o-mini",
                            messages=[{"role": "user", "content": prompt}],
                            response_format={"type": "json_object"}
                        )
                        company_names = json.loads(response.choices[0].message.content).get("names", [])
        ...
        async with AsyncWebCrawler(config=config) as crawler:
            ...
            for link in soup.find_all('a', href=True):
                ...
                if not full_url.startswith(('http://', 'https://')) or urlparse(full_url).netloc != urlparse(url).netloc:
                    continue
                skip_patterns = ['blog', 'news', 'careers', 'jobs', 'press', 'media', 'login', 'signup', 'privacy', 'terms']
            ...
            relevant_pages.extend([page_url for page_url, _ in scored_links[:5]]) 
...
async def process_startup(url: str, name: str, openai_key: str, model: str) -> StartupData:
    ...
    try:
        ...
        async with AsyncWebCrawler(config=config) as crawler:
            async def crawl_page(page_url: str):
                try:
                    ...
                    if result.markdown or result.html:
                        ...
                        regex_result = extract_with_regex(result.markdown or "", result.html or "")
            ...
            contact_tasks = [crawl_contact_page(path) for path in ['/contact', '/contact-us', '/get-in-touch', '/connect']]
        ...
        logger.info(f"Extracted {len(startup_data.founders)} founders and {len(startup_data.emails)} emails from {url}")
    except Exception as e:
...
async def run_extraction_workflow(keyword: str, serpapi_key: str, openai_key: str, model: str, max_results: int) -> pd.DataFrame:
    ...
    async def process_batch(batch: List[tuple]):
        batch_tasks = [process_startup(domain, name, openai_key, model) for domain, name in batch]
    ...
    for i in range(0, len(startups), batch_size):
        ...
        status_text.text(f"Processing batch {i//batch_size + 1}/{(len(startups) + batch_size - 1)//batch_size}")
        ...
        for j, startup_data in enumerate(batch_results):
            if isinstance(startup_data, StartupData):
                ...
                if startup_data.founders or startup_data.emails:
                    st.success(f"✅ {startup_data.name}: {len(startup_data.founders)} founders, {len(startup_data.emails)} emails")
                else:
...
def main():
    ...
    with st.sidebar:
        ...
        serpapi_key = st.text_input("SerpAPI Key", type="password", help="Get from serpapi.com")
        openai_key = st.text_input("OpenAI API Key", type="password", help="Get from platform.openai.com")
        model = st.selectbox(
            "OpenAI Model",
                ...
                if serpapi_key:
                    try:
                        ...
                        if response.status_code == 200:
                            ...
                            st.error(f"❌ SerpAPI failed: {response.status_code}")
                    except Exception as e:
        ...
        if st.button("🚀 Start Extraction", type="primary", use_container_width=True):
            if not serpapi_key:
                ...
                with st.spinner("🔍 Extracting founder data..."):
                    df = asyncio.run(run_extraction_workflow(keyword, serpapi_key, openai_key, model, max_results))
                    if not df.empty:
                        st.success(f"✅ Extraction completed! Found data for {len(df[df['Data Found'] == '✅'])} companies.")
                        ...
                        st.warning(" No results found. Try different keywords or check your API keys.")
